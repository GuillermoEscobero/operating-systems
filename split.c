...
int main(int argc, char *argv[]) {
    ...
    return splitBinary(argv[2], argv[3], argv[4], (int) strtol(argv[1], NULL, 10));
...
int splitBinary(char *inputFile, char *youngerOutputFile, char *olderOutputFile, int thresholdAge) {
    ...
    /* We get the input file descriptor of the input file with a read only file permission */
    int fdInput = open(inputFile, O_RDONLY);

    /* We open both output files in order to store the information, the flags allow the creation of the
     * file or addition at the end to a pre existing one */
    ...
    for (i = 0; i < fileRows; i++) {
        ...
        if (auxPerson.age < thresholdAge) {
            /* If the age of the read person is below the threshold we write it in the corresponding file*/
